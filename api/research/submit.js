...
// Body: { instrument, cohort, family_code, email, answers, website (honeypot),
    ...
    if (instrument === "parents") {
      ...
      if (typeof body.id === "string" && /^[0-9a-f-]{36}$/.test(body.id)) {
        const { rows } = await pool.query(
          "update responses set cohort=$2, family_code=$3, status='complete', completed_at=now(), updated_at=now(), answers=$4, email=$5 where id=$1 and instrument='parents' and status='draft' returning id",
          [body.id, cohort, family_code, JSON.stringify(answers), email],
        );
        if (rows[0]) return send(res, 200, { ok: true, id: rows[0].id });
      }
      const { rows } = await pool.query(
        "insert into responses (instrument, cohort, family_code, status, completed_at, answers, email) values ('parents',$1,$2,'complete',now(),$3,$4) returning id",
